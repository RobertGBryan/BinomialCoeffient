...
namespace BinomialCoefficient
{
   public class BinCoeff<T> : BinCoeffBase
   {
      ...
      // Gets set to true in CreatePascalsTriangle if any calculated value is larger than uint.MaxValue.
      ...
      private void CreatePascalsTriangle()
      {
         ...
         for (loopIndex = GroupSizeM3; loopIndex >= 0; loopIndex--)
         {
            ...
            for (loop = startIndex + 1; loop < endIndex; loop++)
            {
               ...
               if ((loop - 1 >= indexArrayPrev.Length) || (indexArrayPrev[loop - 1] > (uint.MaxValue - indexArray[loop - 1])))
               {
      ...
      public uint GetRank(bool sorted, int[] kIndexes, out bool overflow, int groupSize = -1)
      {
         ...
         if (groupSize == 0)
         {
            s = "BinCoeff:GetRank: groupSize equals zero. This is not allowed.";
            ApplicationException ae = new ApplicationException(s);
            throw ae;
         }
         ...
         if (PasTri == null)
         {
            s = "BinCoeff:GetRank: Error - Pascal's Triangle has not been created. ";
            ApplicationException ae = new ApplicationException(s);
            throw ae;
         }
         uint rank = 0, value;
         ...
         for (loopIndex = startIndex; loopIndex < GroupSizeM1; loopIndex++)
         {
            ...
            value = indexArray[n];
            if ((indexArray.Length > n) && (rank <= uint.MaxValue - value))
               rank += value;
            else
            {
               overflow = true;
      ...
      public void GetCombFromRank(uint rank, int[] kIndexes, int numItems = -1, int groupSize = -1)
      {
         ...
         {
            s = "BinCoeff:GetCombFromRank: groupSize > GroupSize or numItems > NumItems. Neither is allowed. Create a new instance instead.";
            ApplicationException ae = new ApplicationException(s);
            throw ae;
         }
         // Handle the n choose 1 case.
         ...
         if (PasTri == null)
         {
            s = "BinCoeff:GetCombFromRank: Pascal's Triangle has not been created.";
            ApplicationException ae = new ApplicationException(s);
            throw ae;
         }
      ...
      public void GetCombFromRankOld(uint rank, int[] kIndexes, int numItems = -1, int groupSize = -1)
      {
         // This function returns the combination in kIndexes from the specified rank.
         // This is the reverse of the GetRank method. The correct combination is returned in descending order
         // in kIndexes.
         ...
         int loop, startIndex = GroupSize - groupSize;
         uint remValue = rank;
         uint[] indexArray;
         for (loopIndex = startIndex; loopIndex < GroupSizeM1; loopIndex++)
         {
            indexArray = PasTri[loopIndex];
            // If remValue is zero, then look for the last zero value in the array.
            for (loop = indexArray.Length - 1; loop >= 0; loop--)
            {
               if (remValue >= indexArray[loop])
               {
                  kIndexes[loopIndex - startIndex] = loop;
